    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Map> {
        ...
        protected Map doInBackground(Void... params) {
            ...
            try {
                ...
                map = HttpUtils.post(HttpUtils.SERVLET_LOGIN_REQUEST, jsonObject.toString());
...
//            return true;
        ...
        @Override
        protected void onPostExecute(final Map map) {
            mAuthTask = null;
            showProgress(false);

            boolean flag = (boolean) map.get("flag");
            if (flag) {
                boolean success = Boolean.parseBoolean(map.get("data").toString());
                if (success) {
                    SharedPreferencesUtils.sharedPreferencesWrite(getBaseContext(), SharedPreferencesUtils.USER, SharedPreferencesUtils.KEY_Name, mName);
                    finish();
                } else {
