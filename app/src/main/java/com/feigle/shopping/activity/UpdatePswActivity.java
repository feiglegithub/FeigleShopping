...
public class UpdatePswActivity extends AppCompatActivity {
    ...
    private void initView() {
        ...
        oldEditText = findViewById(R.id.oldEditText);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.commitButton:
                String old = oldEditText.getText().toString();
                String newPsw = newEditText.getText().toString();
                String confirm = confirmEditText.getText().toString();

                if (old.equals("")) {
                    oldEditText.setError(getString(R.string.error_field_required));
                    return;
                }
                if (newPsw.equals("")) {
                    newEditText.setError(getString(R.string.error_field_required));
                    return;
                }
                ...
                new UpdateAsyncTask().execute(HttpUtils.SERVLET_UPDATE_PSW_REQUEST,
                        SharedPreferencesUtils.sharedPreferencesRead(this, SharedPreferencesUtils.USER, SharedPreferencesUtils.KEY_Name, ""), old, newPsw);
                break;
            default:
                break;
        }
    ...
    class UpdateAsyncTask extends AsyncTask<String, Integer, Map> {
        ...
        protected Map doInBackground(String... strings) {
            ...
            try {
                ...
                jsonObject.put("old", strings[2]);
                jsonObject.put("newPsw", strings[3]);
